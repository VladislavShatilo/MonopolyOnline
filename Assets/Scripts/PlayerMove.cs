...
public class PlayerMove : MonoBehaviourPun
{
    ...
    private void Awake()
    {
        // Получаем данные инстанциации (colorIndex) если они были переданы PhotonNetwork.Instantiate
    ...
    private void Start()
    {
        // Установим id по владельцу, если не выставлен
        if (id == 0 && photonView.Owner != null)
            id = photonView.Owner.ActorNumber;

        // Попробуем назначить родителя UI/Canvas если он задан в GameManager (делаем это на всех клиентах, чтобы иерархия была одинаковой)
        ...
        // Применим цвет (тот, что пришёл в instantiationData, иначе по ActorNumber)
        ApplySkinColor();

        // Попытаемся инициализировать клетки доски (на всех клиентах)
    ...
    {
        // перед запуском корутины убедимся, что boardCells инициализированы
        EnsureBoardCellsInitialized();

        if (boardCells == null || boardCells.Count == 0)
        {
            Debug.LogError($"[{name}] RPC_MoveSteps: boardCells не инициализированы, отменяю движение.");
            return;
        }
    ...
    private void EnsureBoardCellsInitialized()
    {
        ...
        // Попробуем использовать root, если он уже есть
        ...
        // Попробуем взять из GameManager
        ...
        // если всё ещё нет — выдаём предупреждение
        if (boardCells.Count == 0)
        {
            Debug.LogWarning($"[{name}] Не удалось инициализировать boardCells. Убедись, что GameManager.CellsRootTransforms задан или в сцене есть объект 'CellsRoot'.");
        }
    }
